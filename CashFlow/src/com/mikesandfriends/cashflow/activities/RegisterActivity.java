    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        public void onClick(View v) {
            ...
            if(username.length() >= 1 && password.length() >= 1){
                if(lh.isValidUsername(username.getText().toString().toLowerCase().trim())){
                    User user = new User(username.getText().toString().toLowerCase().trim(),password.getText().toString());
                    lh.add(user);
                    startActivity(i);
                    lh.closeAdapt();
                    finish();
                ...
                CharSequence text = "Username and Password Must be at least 1 character long";
                   int duration = Toast.LENGTH_SHORT;
                username.setText("");
                password.setText("");
                Toast.makeText(getBaseContext(), text, duration).show();
